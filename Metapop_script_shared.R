...
#Otherwise as model 10

#Model 11ny
#Time-dependence for p at site 9 and 10, in years when there are resightings+recaptures
#Otherwise as model 10.tau


#Model xny - invalid parent error (so far 100% of trials)
#Built on model 11ny
#A different parametrization for encounters at site 9 and 10 (where recaptures AND resightings took place)
#Note that while p is used in this model too, the meaning is different from in model 7-10 (and 11ny).
#here p refers to recapture probability


#Model x - invalid parent error (~ 90 % of the trials)
#Same mu and tau for p all years, built on model 9
#A different parametrization for encounters at site 9 and 10 (where recaptures AND resightings took place)
#Note that while p is used in this model too, the meaning is different from in model 7-9.
#here p refers to recapture probability
...
########################
#######################
#It should be possible to run the code on rows 60 - ~175 and then start skip many rows and continue where
#model 10 starts (data, indices etc are being set up here), row 3097
...
cat("
    ...
    for (isl in p.all.years){
    for (t in 1:(n.occasions-1)){
    eps.p[isl, t] ~ dnorm(0, tau.p[in.p[t]])}}
    for (isln in p.all.years){  #Back-transform to the probability scale
    for (stage in c(2:4,6)){ 
    for (t in 1:(n.occasions-1)){
    p.real[isln, stage, t] <- 1/(1+exp(-(mu.p[in.p[t]] + eps.p[isln,t] + delp[stage])))    }}}
    ...
    logit(p[2, stage, t]) <- mu.p[in.p[t]] + eps.p[2, t] + delp[stage]   }}
    for (t in 1:(n.occasions-1)){
    eps.p[2, t] ~ dnorm(0, tau.p[in.p[t]])}
    for (stage in c(2:4,6)){ #Back-transform to the probability scale
    for (t in 1:(n.occasions-1)){
    p.real[2, stage, t] <- 1/(1+exp(-(mu.p[in.p[t]] + eps.p[2,t] + delp[stage])))    }}
    #Site 4 (should have p estimated for 2009-2011,2013-2017):
    ...
    logit(p[4, stage, t]) <- mu.p[in.p[t]] + eps.p[4, t] + delp[stage]   }}
    for (t in c((n.occasions-9):(n.occasions-7), (n.occasions-5):(n.occasions-1))){
    eps.p[4, t] ~ dnorm(0, tau.p[in.p[t]])}
    for (stage in c(2:4,6)){ #Back-transform to the probability scale
    for (t in c((n.occasions-9):(n.occasions-7), (n.occasions-5):(n.occasions-1))){
    p.real[4, stage, t] <- 1/(1+exp(-(mu.p[in.p[t]] + eps.p[4,t] + delp[stage])))    }}
    for (stage in c(2:4,6)){ 
    ...
    logit(p[5, stage, t]) <- mu.p[in.p[t]] + eps.p[5, t] + delp[stage]   }}
    for (t in 1:(n.occasions-1)){
    eps.p[5, t] ~ dnorm(0, tau.p[in.p[t]])}
    for (stage in c(2:4,6)){ #Back-transform to the probability scale
    for (t in 1:(n.occasions-1)){
    p.real[5, stage, t] <- 1/(1+exp(-(mu.p[in.p[t]] + eps.p[5,t] + delp[stage])))    }}
    #Site 6 (should have p estimated for 2011-2017):
    ...
    logit(p[6, stage, t]) <- mu.p[in.p[t]] + eps.p[6, t] + delp[stage]   }}
    for (t in (n.occasions-7):(n.occasions-1)){
    eps.p[6, t] ~ dnorm(0, tau.p[in.p[t]])}
    for (stage in c(2:4,6)){ #Back-transform to the probability scale
    for (t in (n.occasions-7):(n.occasions-1)){
    p.real[6, stage, t] <- 1/(1+exp(-(mu.p[in.p[t]] + eps.p[6,t] + delp[stage])))    }}
    for (stage in c(2:4,6)){ 
    ...
    logit(p[7, stage, t]) <- mu.p[in.p[t]] + eps.p[7, t] + delp[stage]   }}
    for (t in 1:(n.occasions-1)){
    eps.p[7, t] ~ dnorm(0, tau.p[in.p[t]])}
    for (stage in c(2:4,6)){ #Back-transform to the probability scale
    for (t in 1:(n.occasions-1)){
    p.real[7, stage, t] <- 1/(1+exp(-(mu.p[in.p[t]] + eps.p[7,t] + delp[stage])))    }}
    ...
    mean.p[u] ~ dunif(0, 1)                       # Prior for mean re-encounter
    mu.p[u] <- log(mean.p[u] / (1-mean.p[u]))     # Logit transformation
    sigma.p[u] ~ dunif(0, 5)                      # Prior for standard deviation
    tau.p[u] <- pow(sigma.p[u], -2)
    sigma2.p[u] <- pow(sigma.p[u], 2)
    sigma2.p.real[u] <- sigma2.p[u] * pow(mean.p[u], 2) * pow((1-mean.p[u]), 2)}    # Temporal variance on real scale
    #delp[1] <- 0 #Re-encounters do not happen for age 1 [Shouldn't be necessary to define when using stage c(2:4,6) everywhere for p]
    #delp[5] <- 0 #Re-encounters do not happen for full-grown, just marked [Shouldn't be necessary to define when using stage c(2:4,6) everywhere for p]
    delp[2] <- 0 #Baseline age-class, which mu.p[in.p[t]] applies to
    for (stag in c(3,4)){ # Priors for age/stage-effect
    ...
    logit(p10[stage, t]) <- mu.p[in.p[t]] + eps.p10[t] + delp[stage]   }}
    for (t in 1:(n.occasions-1)){
    eps.p10[t] ~ dnorm(0, tau.p[in.p[t]])}
    for (stage in c(2:4,6)){ #Back-transform to the probability scale
    for (t in 1:(n.occasions-1)){
    p10.real[stage, t] <- 1/(1+exp(-(mu.p[in.p[t]] + eps.p10[t] + delp[stage])))    }}
...
inits <- function(){list(a.del=rep(0.5, length.out=3),   
                         mean.phi = 0.1, sigma.phi = runif(1, 0, 5),
                         mean.p = runif(2,min=0,max=1), sigma.p = runif(2, 0, 5), A.delp=rnorm(3),
                         t.g9=rnorm(4), a.g.3 = 0.5,a.g.4 = 0.5,a.g.6 = 0.5, t.g10 = 0.5,
                         B=rnorm(n=1, mean=-0.3, sd=0.1),
                         theta2 = rnorm(1), theta3 = rnorm(1), theta4 = rnorm(1), thetaAa = rnorm(1), thetaAb = rnorm(1), repuls=rnorm(10), attract=rnorm(10)
)}  
...
# Call JAGS from R, using jagsUI syntax
starttime <- Sys.time()
m10.tau <- jags(jags.data, inits, parameters, "model10tau.jags", n.chains = nc, n.thin = nt, n.iter = ni, n.burnin = nb, parallel=TRUE)
endtime <- Sys.time()
options(max.print=999999) #Default is 99999
print(m10.tau, digits = 3) # 
print(m10.tau$summary[1:400,], digits = 3)
...
cat("
    ...
    logit(p9[stage, t]) <- fix.t.p9[t] + delp[stage]   }}
    for (t in (n.occasions-4):(n.occasions-1)){
    fix.t.p9[t] <- fixed.t.p9[t-n.occasions+5]
    fixed.t.p9[t-n.occasions+5] ~ dnorm(0, 0.01)}
    for (stage in c(2:4,6)){ #Back-transform to the probability scale
    for (t in (n.occasions-4):(n.occasions-1)){
    p9.real[stage, t] <- 1/(1+exp(-(fix.t.p9[t] + delp[stage])))    }}
    #Site 10 (should have p estimated for all years in period 2008-2017):
    for (stage in c(2:4,6)){       
    for (t in c(1:(n.occasions-4),(n.occasions-2),(n.occasions-1))){
    logit(p10[stage, t]) <- mu.p[in.p[t]] + eps.p10[t] + delp[stage]   }}
    for (t in c(1:(n.occasions-4),(n.occasions-2),(n.occasions-1))){
    eps.p10[t] ~ dnorm(0, tau.p[in.p[t]])}
    for (stage in c(2:4,6)){ #Back-transform to the probability scale
    ...
    p10.real[stage, t] <- 1/(1+exp(-(mu.p[in.p[t]] + eps.p10[t] + delp[stage])))    }}
    for (stage in c(2:4,6)){      
    for (t in (n.occasions-3)){
    logit(p10[stage, t]) <- fixed.t.p10 + delp[stage]   }}
    fixed.t.p10 ~ dnorm(0, 0.01)
    for (stage in c(2:4,6)){ #Back-transform to the probability scale
    for (t in (n.occasions-3)){
    p10.real[stage, t] <- 1/(1+exp(-(fixed.t.p10 + delp[stage])))    }}
...
inits <- function(){list(a.del=rep(0.5, length.out=3),   
                         mean.phi = 0.1, sigma.phi = runif(1, 0, 5),
                         mean.p = runif(2,min=0,max=1), sigma.p = runif(2, 0, 5), A.delp=rnorm(3), fixed.t.p9=rnorm(4), fixed.t.p10=rnorm(1),
                         t.g9=rnorm(4), a.g.3 = 0.5,a.g.4 = 0.5,a.g.6 = 0.5, t.g10 = 0.5,
                         B=rnorm(n=1, mean=-0.3, sd=0.1),
                         theta2 = rnorm(1), theta3 = rnorm(1), theta4 = rnorm(1), thetaAa = rnorm(1), thetaAb = rnorm(1), repuls=rnorm(10), attract=rnorm(10)
)}  
...
# Call JAGS from R, using jagsUI syntax
starttime <- Sys.time()
m11ny <- jags(jags.data, inits, parameters, "model11ny.jags", n.chains = nc, n.thin = nt, n.iter = ni, n.burnin = nb, parallel=TRUE)
endtime <- Sys.time()
options(max.print=999999) #Default is 99999
print(m11ny, digits = 3) # 
print(m11ny$summary[1:400,], digits = 3)
...
cat("
    ...
    # phi[fro, stage, t]: survival probability at age/stage, having been at site fro, in period t
    # psi[fro, till]: movement probability from site fro to site till, only 10 of these as stage at next time-step is known and thereby constraints options to the 10 sites
    # p[isl, stage, t]: RECAPTURE probability 
    # res[isl, stage, t]: RESIGHTING probability (resightings are modelled at site 9 (last four years of the study) and 10 (in one year only))
    # -------------------------------------------------
    # States (S):
    # 1 alive at a1 s1
    # 2 alive at a2 s1
    ...
    res9[fd, st] <- 0  }}
    for (fd in (n.occasions-4):(n.occasions-1)){
    logit(res9[fd, 2]) <- t.res9[fd-n.occasions+5]
    logit(res9[fd, 3]) <- t.res9[fd-n.occasions+5] + a.res.3
    logit(res9[fd, 4]) <- t.res9[fd-n.occasions+5] + a.res.4
    logit(res9[fd, 6]) <- t.res9[fd-n.occasions+5] + a.res.6
    t.res9[fd-n.occasions+5] ~ dnorm(0, 0.001)
    res9.real[fd,2] <- 1/(1+exp(-(t.res9[fd-n.occasions+5]))) 
    res9.real[fd,3] <- 1/(1+exp(-(t.res9[fd-n.occasions+5] + a.res.3)))
    res9.real[fd,4] <- 1/(1+exp(-(t.res9[fd-n.occasions+5] + a.res.4)))
    res9.real[fd,6] <- 1/(1+exp(-(t.res9[fd-n.occasions+5] + a.res.6)))}
    for (fd in 1:(n.occasions-4)){ 
    for (st in c(2:4,6)){
    res10[fd, st] <- 0  }}
    logit(res10[n.occasions-3, 2]) <- t.res10
    logit(res10[n.occasions-3, 3]) <- t.res10 + a.res.3
    logit(res10[n.occasions-3, 4]) <- t.res10 + a.res.4
    logit(res10[n.occasions-3, 6]) <- t.res10 + a.res.6
    t.res10 ~ dnorm(0, 0.001)
    res10.real[n.occasions-3,2] <- 1/(1+exp(-(t.res10))) 
    res10.real[n.occasions-3,3] <- 1/(1+exp(-(t.res10 + a.res.3)))
    res10.real[n.occasions-3,4] <- 1/(1+exp(-(t.res10 + a.res.4)))
    res10.real[n.occasions-3,6] <- 1/(1+exp(-(t.res10 + a.res.6)))
    for (fd in (n.occasions-2):(n.occasions-1)){ 
    for (st in c(2:4,6)){
    res10[fd, st] <- 0  }}
    a.res.3 ~ dnorm(0, 0.001)
    a.res.4 ~ dnorm(0, 0.001)
    a.res.6 ~ dnorm(0, 0.001)
    ...
    psi[(6*(sf-1))+1,t,50] <- phi[sf,1,t]*psi1[sf,9]*p9[2,t]
    psi[(6*(sf-1))+1,t,51] <- phi[sf,1,t]*psi1[sf,9]*res9[t, 2]
    psi[(6*(sf-1))+1,t,52] <- phi[sf,1,t]*psi1[sf,9]*p9[2,t]*res9[t, 2]
    psi[(6*(sf-1))+1,t,53] <- phi[sf,1,t]*psi1[sf,9]*(1-p9[2,t]-res9[t,2]-p9[2,t]*res9[t,2])
    for (ex in c(54:61,63:66)){
    psi[(6*(sf-1))+1,t,ex] <- 0  }
    psi[(6*(sf-1))+1,t,68] <- phi[sf,1,t]*psi1[sf,10]*p10[2,t]
    psi[(6*(sf-1))+1,t,69] <- phi[sf,1,t]*psi1[sf,10]*res10[t, 2]
    psi[(6*(sf-1))+1,t,70] <- phi[sf,1,t]*psi1[sf,10]*p10[2,t]*res10[t, 2]
    psi[(6*(sf-1))+1,t,71] <- phi[sf,1,t]*psi1[sf,10]*(1-p10[2,t]-res10[t,2]-p10[2,t]*res10[t,2])
    for (ex in c(72:79,81:84)){
    psi[(6*(sf-1))+1,t,ex] <- 0  }
    
    for (ew in 50:53){
    psi[(6*(sf-1))+2,t,ew] <- 0}
    psi[(6*(sf-1))+2,t,54] <- phi[sf,2,t]*psi2[sf,9]*p9[3,t]
    psi[(6*(sf-1))+2,t,55] <- phi[sf,2,t]*psi2[sf,9]*res9[t, 3]
    psi[(6*(sf-1))+2,t,56] <- phi[sf,2,t]*psi2[sf,9]*p9[3,t]*res9[t, 3]
    psi[(6*(sf-1))+2,t,57] <- phi[sf,2,t]*psi2[sf,9]*(1-p9[3,t]-res9[t,3]-p9[3,t]*res9[t,3])
    for (er in c(58:61,63:66)){
    psi[(6*(sf-1))+2,t,er] <- 0  }
    for (ew in 68:71){
    psi[(6*(sf-1))+2,t,ew] <- 0}
    psi[(6*(sf-1))+2,t,72] <- phi[sf,2,t]*psi2[sf,10]*p10[3,t]
    psi[(6*(sf-1))+2,t,73] <- phi[sf,2,t]*psi2[sf,10]*res10[t, 3]
    psi[(6*(sf-1))+2,t,74] <- phi[sf,2,t]*psi2[sf,10]*p10[3,t]*res10[t, 3]
    psi[(6*(sf-1))+2,t,75] <- phi[sf,2,t]*psi2[sf,10]*(1-p10[3,t]-res10[t,3]-p10[3,t]*res10[t,3])
    for (er in c(76:79,81:84)){
    psi[(6*(sf-1))+2,t,er] <- 0  }
    
    for (ed in 50:57){
    psi[(6*(sf-1))+3,t,ed] <- 0}
    psi[(6*(sf-1))+3,t,58] <- phi[sf,3,t]*psi3[sf,9]*p9[4,t]
    psi[(6*(sf-1))+3,t,59] <- phi[sf,3,t]*psi3[sf,9]*res9[t, 4]
    psi[(6*(sf-1))+3,t,60] <- phi[sf,3,t]*psi3[sf,9]*p9[4,t]*res9[t, 4]
    psi[(6*(sf-1))+3,t,61] <- phi[sf,3,t]*psi3[sf,9]*(1-p9[4,t]-res9[t,4]-p9[4,t]*res9[t,4])
    for (ef in 63:66){
    psi[(6*(sf-1))+3,t,ef] <- 0 }
    for (ed in 68:75){
    psi[(6*(sf-1))+3,t,ed] <- 0}
    psi[(6*(sf-1))+3,t,76] <- phi[sf,3,t]*psi3[sf,10]*p10[4,t]
    psi[(6*(sf-1))+3,t,77] <- phi[sf,3,t]*psi3[sf,10]*res10[t, 4]
    psi[(6*(sf-1))+3,t,78] <- phi[sf,3,t]*psi3[sf,10]*p10[4,t]*res10[t, 4]
    psi[(6*(sf-1))+3,t,79] <- phi[sf,3,t]*psi3[sf,10]*(1-p10[4,t]-res10[t,4]-p10[4,t]*res10[t,4])
    for (ef in 81:84){
    psi[(6*(sf-1))+3,t,ef] <- 0 }
    
    for (es in 50:61){
    psi[(6*(sf-1))+4,t,es] <- 0}
    psi[(6*(sf-1))+4,t,63] <- phi[sf,4,t]*psi4[sf,9]*p9[6,t]
    psi[(6*(sf-1))+4,t,64] <- phi[sf,4,t]*psi4[sf,9]*res9[t, 6]
    psi[(6*(sf-1))+4,t,65] <- phi[sf,4,t]*psi4[sf,9]*p9[6,t]*res9[t, 6]
    psi[(6*(sf-1))+4,t,66] <- phi[sf,4,t]*psi4[sf,9]*(1-p9[6,t]-res9[t,6]-p9[6,t]*res9[t,6])
    for (es in 68:79){
    psi[(6*(sf-1))+4,t,es] <- 0}
    psi[(6*(sf-1))+4,t,81] <- phi[sf,4,t]*psi4[sf,10]*p10[6,t]
    psi[(6*(sf-1))+4,t,82] <- phi[sf,4,t]*psi4[sf,10]*res10[t, 6]
    psi[(6*(sf-1))+4,t,83] <- phi[sf,4,t]*psi4[sf,10]*p10[6,t]*res10[t, 6]
    psi[(6*(sf-1))+4,t,84] <- phi[sf,4,t]*psi4[sf,10]*(1-p10[6,t]-res10[t,6]-p10[6,t]*res10[t,6])
    
    for (es in 50:61){
    psi[(6*(sf-1))+5,t,es] <- 0}
    psi[(6*(sf-1))+5,t,63] <- phi[sf,5,t]*psiAa[sf,9]*p9[6,t]
    psi[(6*(sf-1))+5,t,64] <- phi[sf,5,t]*psiAa[sf,9]*res9[t, 6]
    psi[(6*(sf-1))+5,t,65] <- phi[sf,5,t]*psiAa[sf,9]*p9[6,t]*res9[t, 6]
    psi[(6*(sf-1))+5,t,66] <- phi[sf,5,t]*psiAa[sf,9]*(1-p9[6,t]-res9[t,6]-p9[6,t]*res9[t,6])
    for (es in 68:79){
    psi[(6*(sf-1))+5,t,es] <- 0}
    psi[(6*(sf-1))+5,t,81] <- phi[sf,5,t]*psiAa[sf,10]*p10[6,t]
    psi[(6*(sf-1))+5,t,82] <- phi[sf,5,t]*psiAa[sf,10]*res10[t, 6]
    psi[(6*(sf-1))+5,t,83] <- phi[sf,5,t]*psiAa[sf,10]*p10[6,t]*res10[t, 6]
    psi[(6*(sf-1))+5,t,84] <- phi[sf,5,t]*psiAa[sf,10]*(1-p10[6,t]-res10[t,6]-p10[6,t]*res10[t,6])
    
    for (es in 50:61){
    psi[(6*(sf-1))+6,t,es] <- 0}
    psi[(6*(sf-1))+6,t,63] <- phi[sf,6,t]*psiAb[sf,9]*p9[6,t]
    psi[(6*(sf-1))+6,t,64] <- phi[sf,6,t]*psiAb[sf,9]*res9[t, 6]
    psi[(6*(sf-1))+6,t,65] <- phi[sf,6,t]*psiAb[sf,9]*p9[6,t]*res9[t, 6]
    psi[(6*(sf-1))+6,t,66] <- phi[sf,6,t]*psiAb[sf,9]*(1-p9[6,t]-res9[t,6]-p9[6,t]*res9[t,6])
    for (es in 68:79){
    psi[(6*(sf-1))+6,t,es] <- 0}
    psi[(6*(sf-1))+6,t,81] <- phi[sf,6,t]*psiAb[sf,10]*p10[6,t]
    psi[(6*(sf-1))+6,t,82] <- phi[sf,6,t]*psiAb[sf,10]*res10[t, 6]
    psi[(6*(sf-1))+6,t,83] <- phi[sf,6,t]*psiAb[sf,10]*p10[6,t]*res10[t, 6]
    psi[(6*(sf-1))+6,t,84] <- phi[sf,6,t]*psiAb[sf,10]*(1-p10[6,t]-res10[t,6]-p10[6,t]*res10[t,6])
    } #sf
    ...
    psi[49,t,50] <- phi[9,1,t]*psi1[9,9]*p9[2,t]
    psi[49,t,51] <- phi[9,1,t]*psi1[9,9]*res9[t, 2]
    psi[49,t,52] <- phi[9,1,t]*psi1[9,9]*p9[2,t]*res9[t, 2]
    psi[49,t,53] <- phi[9,1,t]*psi1[9,9]*(1-p9[2,t]-res9[t,2]-p9[2,t]*res9[t,2])
    for (ex in c(54:61,63:66)){
    psi[49,t,ex] <- 0  }
    ...
    for (ew in 50:53){
    psi[49+i,t,ew] <- 0  }
    psi[49+i,t,54] <- phi[9,2,t]*psi2[9,9]*p9[3,t]
    psi[49+i,t,55] <- phi[9,2,t]*psi2[9,9]*res9[t, 3]
    psi[49+i,t,56] <- phi[9,2,t]*psi2[9,9]*p9[3,t]*res9[t, 3]
    psi[49+i,t,57] <- phi[9,2,t]*psi2[9,9]*(1-p9[3,t]-res9[t,3]-p9[3,t]*res9[t,3])
    for (er in c(58:61,63:66)){
    psi[49+i,t,er] <- 0  }
    for (ed in 50:57){
    psi[53+i,t,ed] <- 0}
    psi[53+i,t,58] <- phi[9,3,t]*psi3[9,9]*p9[4,t]
    psi[53+i,t,59] <- phi[9,3,t]*psi3[9,9]*res9[t, 4]
    psi[53+i,t,60] <- phi[9,3,t]*psi3[9,9]*p9[4,t]*res9[t, 4]
    psi[53+i,t,61] <- phi[9,3,t]*psi3[9,9]*(1-p9[4,t]-res9[t,4]-p9[4,t]*res9[t,4])
    for (ef in 63:66){
    psi[53+i,t,ef] <- 0 }
    for (es in 50:61){
    psi[57+i,t,es] <- 0}
    psi[57+i,t,63] <- phi[9,4,t]*psi4[9,9]*p9[6,t]
    psi[57+i,t,64] <- phi[9,4,t]*psi4[9,9]*res9[t, 6]
    psi[57+i,t,65] <- phi[9,4,t]*psi4[9,9]*p9[6,t]*res9[t, 6]
    psi[57+i,t,66] <- phi[9,4,t]*psi4[9,9]*(1-p9[6,t]-res9[t,6]-p9[6,t]*res9[t,6])
    for (es in 50:61){
    psi[62+i,t,es] <- 0}
    psi[62+i,t,63] <- phi[9,6,t]*psiAb[9,9]*p9[6,t]
    psi[62+i,t,64] <- phi[9,6,t]*psiAb[9,9]*res9[t, 6]
    psi[62+i,t,65] <- phi[9,6,t]*psiAb[9,9]*p9[6,t]*res9[t, 6]
    psi[62+i,t,66] <- phi[9,6,t]*psiAb[9,9]*(1-p9[6,t]-res9[t,6]-p9[6,t]*res9[t,6])
    } #i
    
    for (es in 50:61){
    psi[62,t,es] <- 0}
    psi[62,t,63] <- phi[9,5,t]*psiAa[9,9]*p9[6,t]
    psi[62,t,64] <- phi[9,5,t]*psiAa[9,9]*res9[t, 6]
    psi[62,t,65] <- phi[9,5,t]*psiAa[9,9]*p9[6,t]*res9[t, 6]
    psi[62,t,66] <- phi[9,5,t]*psiAa[9,9]*(1-p9[6,t]-res9[t,6]-p9[6,t]*res9[t,6])
    
    
    psi[49,t,50+18] <- phi[9,1,t]*psi1[9,10]*p10[2,t]
    psi[49,t,51+18] <- phi[9,1,t]*psi1[9,10]*res10[t, 2]
    psi[49,t,52+18] <- phi[9,1,t]*psi1[9,10]*p10[2,t]*res10[t, 2]
    psi[49,t,53+18] <- phi[9,1,t]*psi1[9,10]*(1-p10[2,t]-res10[t,2]-p10[2,t]*res10[t,2])
    for (ex in c(54:61,63:66)){
    psi[49,t,ex+18] <- 0  }
    ...
    for (ew in 50:53){
    psi[49+i,t,ew+18] <- 0  }
    psi[49+i,t,54+18] <- phi[9,2,t]*psi2[9,10]*p10[3,t]
    psi[49+i,t,55+18] <- phi[9,2,t]*psi2[9,10]*res10[t, 3]
    psi[49+i,t,56+18] <- phi[9,2,t]*psi2[9,10]*p10[3,t]*res10[t, 3]
    psi[49+i,t,57+18] <- phi[9,2,t]*psi2[9,10]*(1-p10[3,t]-res10[t,3]-p10[3,t]*res10[t,3])
    for (er in c(58:61,63:66)){
    psi[49+i,t,er+18] <- 0  }
    for (ed in 50:57){
    psi[53+i,t,ed+18] <- 0}
    psi[53+i,t,58+18] <- phi[9,3,t]*psi3[9,10]*p10[4,t]
    psi[53+i,t,59+18] <- phi[9,3,t]*psi3[9,10]*res10[t, 4]
    psi[53+i,t,60+18] <- phi[9,3,t]*psi3[9,10]*p10[4,t]*res10[t, 4]
    psi[53+i,t,61+18] <- phi[9,3,t]*psi3[9,10]*(1-p10[4,t]-res10[t,4]-p10[4,t]*res10[t,4])
    for (ef in 63:66){
    psi[53+i,t,ef+18] <- 0 }
    for (es in 50:61){
    psi[57+i,t,es+18] <- 0}
    psi[57+i,t,63+18] <- phi[9,4,t]*psi4[9,10]*p10[6,t]
    psi[57+i,t,64+18] <- phi[9,4,t]*psi4[9,10]*res10[t, 6]
    psi[57+i,t,65+18] <- phi[9,4,t]*psi4[9,10]*p10[6,t]*res10[t, 6]
    psi[57+i,t,66+18] <- phi[9,4,t]*psi4[9,10]*(1-p10[6,t]-res10[t,6]-p10[6,t]*res10[t,6])
    for (es in 50:61){
    psi[62+i,t,es+18] <- 0}
    psi[62+i,t,63+18] <- phi[9,6,t]*psiAb[9,10]*p10[6,t]
    psi[62+i,t,64+18] <- phi[9,6,t]*psiAb[9,10]*res10[t, 6]
    psi[62+i,t,65+18] <- phi[9,6,t]*psiAb[9,10]*p10[6,t]*res10[t, 6]
    psi[62+i,t,66+18] <- phi[9,6,t]*psiAb[9,10]*(1-p10[6,t]-res10[t,6]-p10[6,t]*res10[t,6])
    } #i
    
    for (es in 50:61){
    psi[62,t,es+18] <- 0}
    psi[62,t,63+18] <- phi[9,5,t]*psiAa[9,10]*p10[6,t]
    psi[62,t,64+18] <- phi[9,5,t]*psiAa[9,10]*res10[t, 6]
    psi[62,t,65+18] <- phi[9,5,t]*psiAa[9,10]*p10[6,t]*res10[t, 6]
    psi[62,t,66+18] <- phi[9,5,t]*psiAa[9,10]*(1-p10[6,t]-res10[t,6]-p10[6,t]*res10[t,6])
    
    psi[67,t,50] <- phi[10,1,t]*psi1[10,9]*p9[2,t]
    psi[67,t,51] <- phi[10,1,t]*psi1[10,9]*res9[t, 2]
    psi[67,t,52] <- phi[10,1,t]*psi1[10,9]*p9[2,t]*res9[t, 2]
    psi[67,t,53] <- phi[10,1,t]*psi1[10,9]*(1-p9[2,t]-res9[t,2]-p9[2,t]*res9[t,2])
    for (ex in c(72:79,81:84)){
    psi[67,t,ex-18] <- 0  }
    ...
    for (ew in 68:71){
    psi[67+i,t,ew-18] <- 0  }
    psi[67+i,t,72-18] <- phi[10,2,t]*psi2[10,9]*p9[3,t]
    psi[67+i,t,73-18] <- phi[10,2,t]*psi2[10,9]*res9[t, 3]
    psi[67+i,t,74-18] <- phi[10,2,t]*psi2[10,9]*p9[3,t]*res9[t, 3]
    psi[67+i,t,75-18] <- phi[10,2,t]*psi2[10,9]*(1-p9[3,t]-res9[t,3]-p9[3,t]*res9[t,3])
    for (er in c(76:79,81:84)){
    psi[67+i,t,er-18] <- 0  }
    for (ed in 68:75){
    psi[71+i,t,ed-18] <- 0}
    psi[71+i,t,76-18] <- phi[10,3,t]*psi3[10,9]*p9[4,t]
    psi[71+i,t,77-18] <- phi[10,3,t]*psi3[10,9]*res9[t, 4]
    psi[71+i,t,78-18] <- phi[10,3,t]*psi3[10,9]*p9[4,t]*res9[t, 4]
    psi[71+i,t,79-18] <- phi[10,3,t]*psi3[10,9]*(1-p9[4,t]-res9[t,4]-p9[4,t]*res9[t,4])
    for (ef in 81:84){
    psi[71+i,t,ef-18] <- 0 }
    for (es in 68:79){
    psi[75+i,t,es-18] <- 0}
    psi[75+i,t,81-18] <- phi[10,4,t]*psi4[10,9]*p9[6,t]
    psi[75+i,t,82-18] <- phi[10,4,t]*psi4[10,9]*res9[t, 6]
    psi[75+i,t,83-18] <- phi[10,4,t]*psi4[10,9]*p9[6,t]*res9[t, 6]
    psi[75+i,t,84-18] <- phi[10,4,t]*psi4[10,9]*(1-p9[6,t]-res9[t,6]-p9[6,t]*res9[t,6])
    for (es in 68:79){
    psi[80+i,t,es-18] <- 0}
    psi[80+i,t,81-18] <- phi[10,6,t]*psiAb[10,9]*p9[6,t]
    psi[80+i,t,82-18] <- phi[10,6,t]*psiAb[10,9]*res9[t, 6]
    psi[80+i,t,83-18] <- phi[10,6,t]*psiAb[10,9]*p9[6,t]*res9[t, 6]
    psi[80+i,t,84-18] <- phi[10,6,t]*psiAb[10,9]*(1-p9[6,t]-res9[t,6]-p9[6,t]*res9[t,6])
    } #i
    
    for (es in 68:79){
    psi[80,t,es-18] <- 0}
    psi[80,t,81-18] <- phi[10,5,t]*psiAa[10,9]*p9[6,t]
    psi[80,t,82-18] <- phi[10,5,t]*psiAa[10,9]*res9[t, 6]
    psi[80,t,83-18] <- phi[10,5,t]*psiAa[10,9]*p9[6,t]*res9[t, 6]
    psi[80,t,84-18] <- phi[10,5,t]*psiAa[10,9]*(1-p9[6,t]-res9[t,6]-p9[6,t]*res9[t,6])
    
    
    psi[67,t,68] <- phi[10,1,t]*psi1[10,10]*p10[2,t]
    psi[67,t,69] <- phi[10,1,t]*psi1[10,10]*res10[t, 2]
    psi[67,t,70] <- phi[10,1,t]*psi1[10,10]*p10[2,t]*res10[t, 2]
    psi[67,t,71] <- phi[10,1,t]*psi1[10,10]*(1-p10[2,t]-res10[t,2]-p10[2,t]*res10[t,2])
    for (ex in c(72:79,81:84)){
    psi[67,t,ex] <- 0  }
    ...
    for (ew in 68:71){
    psi[67+i,t,ew] <- 0  }
    psi[67+i,t,72] <- phi[10,2,t]*psi2[10,10]*p10[3,t]
    psi[67+i,t,73] <- phi[10,2,t]*psi2[10,10]*res10[t, 3]
    psi[67+i,t,74] <- phi[10,2,t]*psi2[10,10]*p10[3,t]*res10[t, 3]
    psi[67+i,t,75] <- phi[10,2,t]*psi2[10,10]*(1-p10[3,t]-res10[t,3]-p10[3,t]*res10[t,3])
    for (er in c(76:79,81:84)){
    psi[67+i,t,er] <- 0  }
    for (ed in 68:75){
    psi[71+i,t,ed] <- 0}
    psi[71+i,t,76] <- phi[10,3,t]*psi3[10,10]*p10[4,t]
    psi[71+i,t,77] <- phi[10,3,t]*psi3[10,10]*res10[t, 4]
    psi[71+i,t,78] <- phi[10,3,t]*psi3[10,10]*p10[4,t]*res10[t, 4]
    psi[71+i,t,79] <- phi[10,3,t]*psi3[10,10]*(1-p10[4,t]-res10[t,4]-p10[4,t]*res10[t,4])
    for (ef in 81:84){
    psi[71+i,t,ef] <- 0 }
    for (es in 68:79){
    psi[75+i,t,es] <- 0}
    psi[75+i,t,81] <- phi[10,4,t]*psi4[10,10]*p10[6,t]
    psi[75+i,t,82] <- phi[10,4,t]*psi4[10,10]*res10[t, 6]
    psi[75+i,t,83] <- phi[10,4,t]*psi4[10,10]*p10[6,t]*res10[t, 6]
    psi[75+i,t,84] <- phi[10,4,t]*psi4[10,10]*(1-p10[6,t]-res10[t,6]-p10[6,t]*res10[t,6])
    for (es in 68:79){
    psi[80+i,t,es] <- 0}
    psi[80+i,t,81] <- phi[10,6,t]*psiAb[10,10]*p10[6,t]
    psi[80+i,t,82] <- phi[10,6,t]*psiAb[10,10]*res10[t, 6]
    psi[80+i,t,83] <- phi[10,6,t]*psiAb[10,10]*p10[6,t]*res10[t, 6]
    psi[80+i,t,84] <- phi[10,6,t]*psiAb[10,10]*(1-p10[6,t]-res10[t,6]-p10[6,t]*res10[t,6])
    } #i
    for (es in 68:79){
    psi[80,t,es] <- 0}
    psi[80,t,81] <- phi[10,5,t]*psiAa[10,10]*p10[6,t]
    psi[80,t,82] <- phi[10,5,t]*psiAa[10,10]*res10[t, 6]
    psi[80,t,83] <- phi[10,5,t]*psiAa[10,10]*p10[6,t]*res10[t, 6]
    psi[80,t,84] <- phi[10,5,t]*psiAa[10,10]*(1-p10[6,t]-res10[t,6]-p10[6,t]*res10[t,6])
...
inits <- function(){list(a.del=rep(0.5, length.out=3),   
                         mean.phi = 0.1, sigma.phi = runif(1, 0, 5),
                         mean.p = runif(2,min=0,max=1), sigma.p = runif(2, 0, 5), A.delp=rnorm(3), fixed.t.p9=rnorm(4), fixed.t.p10=rnorm(1),
                         t.res9=rnorm(n=4, mean=0, sd=0.1), a.res.3 = rnorm(n=1, mean=0, sd=0.1), a.res.4 = rnorm(n=1, mean=0, sd=0.1), a.res.6 = rnorm(n=1, mean=0, sd=0.1), t.res10 = rnorm(n=1, mean=0, sd=0.1),
                         B=rnorm(n=1, mean=-0.3, sd=0.1),
                         theta2 = rnorm(1), theta3 = rnorm(1), theta4 = rnorm(1), thetaAa = rnorm(1), thetaAb = rnorm(1), repuls=rnorm(10), attract=rnorm(10)
)}  

# Parameters monitored
parameters <- c("phi.real", "sigma2.phi.real", "p.real", "sigma2.p.real", "p9.real","res9.real", "p10.real", "res10.real","B", "psi1", "psi2","psi3", "psiAa", "psiAb")
...
# Call JAGS from R, using jagsUI syntax
starttime <- Sys.time()
mxny <- jags(jags.data, inits, parameters, "modelXny.jags", n.chains = nc, n.thin = nt, n.iter = ni, n.burnin = nb, parallel=TRUE)
endtime <- Sys.time()
options(max.print=999999) #Default is 99999
print(mxny, digits = 3) # 
print(mxny$summary[1:400,], digits = 3)
...
#########~~~~~~~~~~~~~~###########
phi6.means <- data.frame(col=rep(1:10, each=9),period=rep(1:9, times=10), m8res=rep(NA, times=90), m9res=rep(NA, times=90), m10res=rep(NA, times=90), m10taures=rep(NA, times=90), m11nyres=rep(NA, times=90), mXres=rep(NA, times=90))
for (col in 1:10){
  for (t in 1:9){
    phi6.means[(col-1)*9+t,"m8res"] <- round(m8$mean$phi.real[col, 6, t], digits=3)
    phi6.means[(col-1)*9+t,"m9res"] <- round(m9$mean$phi.real[col, 6, t], digits=3)
    phi6.means[(col-1)*9+t,"m10res"] <- round(m10$mean$phi.real[col, 6, t], digits=3)
    phi6.means[(col-1)*9+t,"m10taures"] <- round(m10.tau$mean$phi.real[col, 6, t], digits=3)
    phi6.means[(col-1)*9+t,"m11nyres"] <- round(m11ny$mean$phi.real[col, 6, t], digits=3)
    phi6.means[(col-1)*9+t,"mXres"] <- round(mX$mean$phi.real[col, 6, t], digits=3)
  } }
phi6.means
phi.diff.11ny <- tapply(phi6.means$m11nyres, list(phi6.means$col), FUN=mean)
...
for (col in 1:10){
  for (t in 1:9){
    ...
    p6.means[(col-1)*9+t,"m10res"] <- round(m10$mean$p.real[col, 6, t], digits=3)
    p6.means[(col-1)*9+t,"m10taures"] <- round(m10.tau$mean$p.real[col, 6, t], digits=3)
    p6.means[(col-1)*9+t,"m11nyres"] <- round(m11ny$mean$p.real[col, 6, t], digits=3)
    p6.means[(col-1)*9+t,"mXres"] <- round(mX$mean$p.real[col, 6, t], digits=3)
  } }
p6.means
p.diff.11ny <- tapply(p6.means$m11nyres, list(p6.means$col), FUN=mean, na.rm=T)
colMeans(p6.means, na.rm=T)

phi5.means <- data.frame(col=rep(1:10, each=9),period=rep(1:9, times=10), m8res=rep(NA, times=90), m9res=rep(NA, times=90), m10res=rep(NA, times=90), m10taures=rep(NA, times=90), m11nyres=rep(NA, times=90), mXres=rep(NA, times=90))
for (col in 1:10){
  for (t in 1:9){
    phi5.means[(col-1)*9+t,"m8res"] <- round(m8$mean$phi.real[col, 5, t], digits=3)
    phi5.means[(col-1)*9+t,"m9res"] <- round(m9$mean$phi.real[col, 5, t], digits=3)
    phi5.means[(col-1)*9+t,"m10res"] <- round(m10$mean$phi.real[col, 5, t], digits=3)
    phi5.means[(col-1)*9+t,"m10taures"] <- round(m10.tau$mean$phi.real[col, 5, t], digits=3)
    phi5.means[(col-1)*9+t,"m11nyres"] <- round(m11ny$mean$phi.real[col, 5, t], digits=3)
    phi5.means[(col-1)*9+t,"mXres"] <- round(mX$mean$phi.real[col, 5, t], digits=3)
  } }
phi5.means
phi5.diff.11ny <- tapply(phi5.means$m11nyres, list(phi5.means$col), FUN=mean)
phi5.diff.x <- tapply(phi5.means$mX, list(phi5.means$col), FUN=mean)
colMeans(phi5.means)

phi1.means <- data.frame(col=rep(1:10, each=9),period=rep(1:9, times=10), m8res=rep(NA, times=90), m9res=rep(NA, times=90), m10res=rep(NA, times=90), m10taures=rep(NA, times=90), m11nyres=rep(NA, times=90), mXres=rep(NA, times=90))
for (col in 1:10){
  for (t in 1:9){
    phi1.means[(col-1)*9+t,"m8res"] <- round(m8$mean$phi.real[col, 1, t], digits=3)
    phi1.means[(col-1)*9+t,"m9res"] <- round(m9$mean$phi.real[col, 1, t], digits=3)
    phi1.means[(col-1)*9+t,"m10res"] <- round(m10$mean$phi.real[col, 1, t], digits=3)
    phi1.means[(col-1)*9+t,"m10taures"] <- round(m10.tau$mean$phi.real[col, 1, t], digits=3)
    phi1.means[(col-1)*9+t,"m11nyres"] <- round(m11ny$mean$phi.real[col, 1, t], digits=3)
    phi1.means[(col-1)*9+t,"mXres"] <- round(mX$mean$phi.real[col, 1, t], digits=3)
  } }
phi1.means
phi1.diff.11ny <- tapply(phi1.means$m11nyres, list(phi1.means$col), FUN=mean)
phi1.diff.x <- tapply(phi1.means$mXres, list(phi1.means$col), FUN=mean)
colMeans(phi1.means)
...
########################
########################
#Model X - only worked twice
########################
#Recoding the resighting at site 9 and 10 - getting rid of the bias in recapture rates, due to 
#encounter probabilities at site 9 and 10 being sampled from the same distribution as at sites 1-8
#despite encounter at 9 and 10 includes resightings in addition to recaptures, thus much higher encounter probabilities
...
cat("
    ...
    # Priors and constraints
    # Survival: random effects of time and island, additive effect of age/stage
    for (fro in 1:10){
    for (stage in 1:6){ 
    for (t in 1:(n.occasions-1)){
    logit(phi[fro, stage, t]) <- mu.phi + eps.phi[fro, t] + del[stage] 
    ...
    # Encounters at site 1-8: random effect of time and island, additive effect of age/stage
    #Sites 1,3,8: 
    for (isl in p.all.years){ 
    for (stage in c(2:4,6)){ 
    for (t in 1:(n.occasions-1)){
    ...
    logit(res9[fd, 4]) <- t.res9[fd-n.occasions+5] + a.res.4
    logit(res9[fd, 6]) <- t.res9[fd-n.occasions+5] + a.res.6
    t.res9[fd-n.occasions+5] ~ dnorm(0, 0.01)
    res9.real[fd,2] <- 1/(1+exp(-(t.res9[fd-n.occasions+5]))) 
    res9.real[fd,3] <- 1/(1+exp(-(t.res9[fd-n.occasions+5] + a.res.3)))
    res9.real[fd,4] <- 1/(1+exp(-(t.res9[fd-n.occasions+5] + a.res.4)))
    res9.real[fd,6] <- 1/(1+exp(-(t.res9[fd-n.occasions+5] + a.res.6)))}
    ...
    for (st in c(2:4,6)){
    res10[fd, st] <- 0  }}
    a.res.3 ~ dnorm(0, 0.01)
    a.res.4 ~ dnorm(0, 0.01)
    a.res.6 ~ dnorm(0, 0.01)
...
inits <- function(){list(a.del=rep(0.5, length.out=3),   
                         mean.phi = 0.1, sigma.phi = runif(1, 0, 5),
                         mean.p = 0.1, sigma.p = runif(1, 0, 5), A.delp=rnorm(3),
                         t.res9=rnorm(4), a.res.3 = 0.5,a.res.4 = 0.5,a.res.6 = 0.5, t.res10 = 0.5,
                         B=rnorm(n=1, mean=-0.1, sd=0.1),
                         theta2 = rnorm(1), theta3 = rnorm(1), theta4 = rnorm(1), thetaAa = rnorm(1), thetaAb = rnorm(1), repuls=rnorm(10), attract=rnorm(10)
)}  #This set of initial values have worked, but I have had problems (of the same type as before, "invalid parent values")
...
# Call JAGS from R, using jagsUI syntax
starttime <- Sys.time()
mX <- jags(jags.data, inits, parameters, "modelX.jags", n.chains = nc, n.thin = nt, n.iter = ni, n.burnin = nb, parallel=FALSE)
endtime <- Sys.time()
options(max.print=999999) #Default is 99999
print(mX, digits = 3) #2.1 h at 150 it/3 chains
print(mX$summary[1:400,], digits = 3) #phi for adults hasn't changed much, phi for first year-of-life more realistic (around 50%)
#Only worked twice, otherwise invalid parent values
